...
class Gen1Chooser:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("Gen 1 Solo Challenge")
        self.root.geometry("1000x700")

        button_frame = tk.Frame(self.root)
        button_frame.pack(side=tk.TOP, pady=10)

        self.Pokemon_Blue_button = tk.Button(button_frame, text="Pick Pokemon Blue", 
            command=lambda: self.generate_choice('Blue'))
        self.Pokemon_Blue_button.pack(side=tk.LEFT, padx=10)

        self.Pokemon_Red_button = tk.Button(button_frame, text="Pick Pokemon Red", 
            command=lambda: self.generate_choice('Red'))
        self.Pokemon_Red_button.pack(side=tk.LEFT, padx=10)

        self.Pokemon_Yellow_button = tk.Button(button_frame, text="Pick Pokemon Yellow", 
            command=lambda: self.generate_choice('Yellow'))
        self.Pokemon_Yellow_button.pack(side=tk.LEFT, padx=10)
