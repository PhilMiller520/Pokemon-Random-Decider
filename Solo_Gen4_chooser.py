...
class Gen4Chooser:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("Gen 4 Solo Challenge")
        self.root.geometry("1000x700")

        button_frame = tk.Frame(self.root)
        button_frame.pack(side=tk.TOP, pady=10)

        self.Pokemon_Diamond_button = tk.Button(button_frame, text="Pick Pokemon Diamond", 
            command=lambda: self.generate_choice('Diamond'))
        self.Pokemon_Diamond_button.pack(side=tk.LEFT, padx=10)

        self.Pokemon_Pearl_button = tk.Button(button_frame, text="Pick Pokemon Pearl", 
            command=lambda: self.generate_choice('Pearl'))
        self.Pokemon_Pearl_button.pack(side=tk.LEFT, padx=10)

        self.Pokemon_Platinum_button = tk.Button(button_frame, text="Pick Pokemon Platinum", 
            command=lambda: self.generate_choice('Platinum'))
        self.Pokemon_Platinum_button.pack(side=tk.LEFT, padx=10)
        
        self.Pokemon_HeartGold_button = tk.Button(button_frame, text="Pick Pokemon Heart Gold", 
            command=lambda: self.generate_choice('Heart Gold'))
        self.Pokemon_HeartGold_button.pack(side=tk.LEFT, padx=10)
        
        self.Pokemon_SoulSilver_button = tk.Button(button_frame, text="Pick Pokemon Soul Silver", 
            command=lambda: self.generate_choice('Soul Silver'))
        self.Pokemon_SoulSilver_button.pack(side=tk.LEFT, padx=10)
